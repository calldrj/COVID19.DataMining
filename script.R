...
# 4. Remove some irrelevant rows (18:19, 81:82, 144:145, 208:209, 271:272, 334:335),
#    Then reset the row index of the data table
cv19_raw <- cv19_raw[-c(18:19, 81:82, 144:145, 208:209, 271:272, 334:335), ]
rownames(cv19_raw) <- NULL

# 5. Review the structure and content of the raw data table
str(cv19_raw)
View(cv19_raw)

# 6. Save the data of confirmed, deaths, and recovered cases from the raw table
#    Set border of the states' data table (top, bottom [the last updated row], left, right) 
#    Row #345 corresponds with date 22-Jan-21
...
# 7. Visible review the structure and content of the saved data table
str(cv19_us)
View(cv19_us)
str(cv19_st)
View(cv19_st)
...
colnames(cv19_us)[c(3, 5, 7, 8)] <- c('Cumulated_Confirmed', 'Cumulated_Deaths', 'Cumulated_Recovered', 'Active')
...
# 9. Fill each of emtpy cells with 0
col <- c(2:8)
...
sapply(cv19_us[1:8], class)

# 11. Convert data in columns, except Date of both tables to number
library(stringr)    # str_remove_all (commas)
col <- c(2:8)
cv19_us[ , col] <- apply(cv19_us[ , col], 2, function(x) as.numeric(str_remove_all(x, '[,]')))
col <- c(2:56)
cv19_st[ , col] <- apply(cv19_st[ , col], 2, function(x) as.numeric(str_remove_all(x, '[,]')))
...
cv19_us$Date <- as.Date(cv19_us$Date, format="%d-%b-%y")
cv19_st$Date <- as.Date(cv19_st$Date, format="%d-%b-%y")
...
# 2. Pie chart shows all cases ('Active': p, 'Death': d, 'Recovered'; r)
p <- sum(df_us$Cumulated_Recovered[lr])
d <- sum(df_us$Cumulated_Deaths[lr])
r <- sum(df_us$Active[lr])
case <- c('Recovered','Death', 'Active')
case_number <- c(p, d, r)
...
palette_color <- c('Death'='#0D1B2E', 'Recovered'='#429890', 'Active'='#B51C35')
...
# 2. Point charts show all the progression of cases ('Active', 'Deaths' and 'Recovered'; r) over time
library(scales)
library(plotly)
subtitle_con <- paste('Total Active: ', prettyNum(df_us$Active[lr], big.mark=','))
...
plot_active <- ggplot(df_us, aes(x=Date, y=Active)) + scale_y_log10() +
  geom_point(color='#B51C35') +
  geom_smooth( color='#0D1B2E') +
  theme(axis.text.x=element_text(angle=45, hjust=1)) +
  xlab('') +
  scale_x_date(labels=date_format('%b %d')) +
  ylab('Number of Active Cases (log10)') +
  scale_y_continuous(labels=function(x) format(x, scientific=FALSE)) 
ggplotly(plot_active) %>% layout(title=list(text=plot_title, y=1), font=list(family='sans serif', size=18, color='#E95D0F')) %>%
...
plot_dr <- ggplot(df_us, aes(x=Date)) + scale_y_log10() +
...
weekly_count <- aggregate(df_us[ , c(4,6,8)], by=list(Week=week(df_us$Date)), sum)
...
cal_con <- ggplot_calendar_heatmap(df_us, 'Date', 'Active') +
